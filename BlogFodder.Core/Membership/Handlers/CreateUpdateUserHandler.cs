...
namespace BlogFodder.Core.Membership.Handlers
{
    public class CreateUpdateUserHandler : IRequestHandler<CreateUpdateUserCommand, HandlerResult<User>>
    {
        ...
        public async Task<HandlerResult<User>> Handle(CreateUpdateUserCommand request, CancellationToken cancellationToken)
        {
            ...
            var efUser = await _db.Users.FirstOrDefaultAsync(x => x.Id == authState.User.GetUserId(), cancellationToken: cancellationToken).ConfigureAwait(false);
            if (efUser == null)
            {
            ...
            if (request.ProfileImageUpload != null)
            {
                ...
                var fileSaveResult = await _providerService.StorageProvider!.SaveFile(request.ProfileImageUpload, efUser.Id.ToString()).ConfigureAwait(false);
            ...
            if (!managerUser.ExtendedData.Get<bool>(Constants.ExtendedDataKeys.IsExternalLogin))
            {
                ...
                if (email != managerUser.Email)
                {
                    ...
                    if (_userManager.Options.SignIn.RequireConfirmedAccount)
                    {
                        ...
                        handlerResult.RefreshSignIn = true;
                        handlerResult.AddMessage("Please check your email and click the link to confirm your email address", ResultMessageType.Info);
                    }
                    else
                    {
                        // Just generate the code and change the email
                        var code = await _userManager.GenerateChangeEmailTokenAsync(managerUser, email).ConfigureAwait(false);
                        //code = WebEncoders.Base64UrlEncode(Encoding.UTF8.GetBytes(code));
                        //code = Encoding.UTF8.GetString(WebEncoders.Base64UrlDecode(code));
                        var emailResult = await _userManager.ChangeEmailAsync(managerUser, email, code).ConfigureAwait(false);
                ...
                {
                    var changePasswordResult = await _userManager.ChangePasswordAsync(managerUser, request.CurrentPassword, request.NewPassword).ConfigureAwait(false);
                ...
                phoneNumber != managerUser.PhoneNumber)
            {
                ...
                var code = await _userManager.GenerateChangePhoneNumberTokenAsync(managerUser, phoneNumber).ConfigureAwait(false);
                var phoneResult = await _userManager.ChangePhoneNumberAsync(managerUser, phoneNumber, code).ConfigureAwait(false);
