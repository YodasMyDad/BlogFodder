...
public static class EditContextFluentValidationExtensions
{
    ...
    public static void AddFluentValidation(this EditContext editContext, IServiceProvider serviceProvider, IValidator? validator, FluentValidationValidator fluentValidationValidator)
    {
        ...
        editContext.OnValidationRequested +=
            async (sender, _) => await ValidateModel((EditContext)sender!, messages, serviceProvider, fluentValidationValidator, validator);

        editContext.OnFieldChanged +=
            async (_, eventArgs) => await ValidateField(editContext, messages, eventArgs.FieldIdentifier, serviceProvider, validator);
    }
    ...
    private static IValidator? GetValidatorForModel(IServiceProvider serviceProvider, object model)
    {
        ...
        // Replace this with our plugin assemblies
        foreach (var assembly in AssemblyManager.Assemblies.Where(i => i?.FullName is not null && !ScannedAssembly.Contains(i.FullName)))
        {
            if (assembly != null)
            {
                try
